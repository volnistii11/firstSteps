...
class M_Basket
{
    public function getBasket($session_id)
    {
       ...
       catalog.name as catalog_name, catalog.price as catalog_price, basket.session_id as session_id, catalog.id as catalog_id 
        FROM basket, catalog WHERE basket.catalog_id = catalog.id AND session_id = :session_id', ['session_id' => $session_id]);
    }
